...
You can also generate a single DTO that is not an IEnumerable/IQueryable:

```csharp
var foo = 1;
var bar = "hello";
// result: MyDto✨️
var result = LinqraftKit.Generate<MyDto>(
    new {
        Foo = foo,
        Bar = bar,
    },
    // capture required for local variables used in the selector body
    capture: () => (foo, bar)
);
```
