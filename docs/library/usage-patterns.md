...
```
Do you need a named type?
├─ No → Use Anonymous Pattern
└─ Yes
    ├─ Do you already have a DTO class?
    │   ├─ Yes → Use Pre-existing DTO Pattern
    │   └─ No → Use Explicit DTO Pattern
    └─ (or consider if you need to share the DTO across projects)
        ├─ Yes → Use Pre-existing DTO Pattern
        └─ No → Use Explicit DTO Pattern
```
...
All patterns support these features:
...
.SelectExpr<Order, OrderDto>(o => new
{
    // Automatically converted to null checks
    CustomerName = o.Customer?.Name,
...
{
    TotalAmount = o.OrderItems.Sum(oi => oi.Quantity * oi.UnitPrice),
    AveragePrice = o.OrderItems.Average(oi => oi.UnitPrice),
    MaxQuantity = o.OrderItems.Max(oi => oi.Quantity),
})
```
...
* [Customization](./customization.md) - Learn how to customize the generated code
