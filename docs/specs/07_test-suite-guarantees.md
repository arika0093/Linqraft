...
| `tests\Linqraft.Analyzer.Tests` | analyzer and code-fix coverage | `AnonymousTypeToDtoAnalyzerTests.cs`, `SelectToSelectExprNamedCodeFixProviderTests.cs`, `LocalVariableCaptureAnalyzerTests.cs`, `GeneratedHashedNamespaceUsageAnalyzerTests.cs` |
...
| --- | --- | --- |
| `LQRF001` AnonymousTypeToDto | anonymous shapes outside Linqraft contexts are suggested for DTO conversion, with context-sensitive DTO naming | `tests\Linqraft.Analyzer.Tests\AnonymousTypeToDtoAnalyzerTests.cs`, `tests\Linqraft.Analyzer.Tests\AnonymousTypeToDtoCodeFixProviderTests.cs` |
| `LQRF002` ApiControllerProducesResponseType | API controllers using typed `SelectExpr` can receive response-metadata suggestions and fixes | `tests\Linqraft.Analyzer.Tests\ApiControllerProducesResponseTypeAnalyzerTests.cs`, `tests\Linqraft.Analyzer.Tests\ApiControllerProducesResponseTypeCodeFixProviderTests.cs` |
| `LQRF003` / `LQRF004` API response method generation | anonymous `Select`-based mock query methods can be converted into async/sync API response methods | `tests\Linqraft.Analyzer.Tests\ApiResponseMethodGeneratorAnalyzerTests.cs`, `tests\Linqraft.Analyzer.Tests\SyncApiResponseMethodGeneratorAnalyzerTests.cs`, corresponding code-fix tests |
| `LQRS001` SelectExprToTyped | anonymous `SelectExpr` calls can be promoted to `SelectExpr<TSource, TDto>` with inferred DTO naming | `tests\Linqraft.Analyzer.Tests\SelectExprToTypedAnalyzerTests.cs` |
| `LQRS002` anonymous `Select` to `SelectExpr` | only `IQueryable` anonymous projections are targeted, and code fixes convert them to Linqraft forms | `tests\Linqraft.Analyzer.Tests\SelectToSelectExprAnonymousAnalyzerTests.cs`, `tests\Linqraft.Analyzer.Tests\SelectToSelectExprAnonymousCodeFixProviderTests.cs` |
| `LQRS003` named `Select` to `SelectExpr` | named-object projections can be converted through the supported fix variants | `tests\Linqraft.Analyzer.Tests\SelectToSelectExprNamedAnalyzerTests.cs`, `tests\Linqraft.Analyzer.Tests\SelectToSelectExprNamedCodeFixProviderTests.cs` |
| `LQRS004` ternary simplification | null-guard ternaries with object creation inside `SelectExpr` are flagged and simplified via code fix | `tests\Linqraft.Analyzer.Tests\TernaryNullCheckToConditionalAnalyzerTests.cs`, `tests\Linqraft.Analyzer.Tests\TernaryNullCheckToConditionalCodeFixProviderTests.cs` |
| `LQRS005` unnecessary capture | unused capture entries are diagnosed and can be removed automatically | `tests\Linqraft.Analyzer.Tests\UnnecessaryCaptureAnalyzerTests.cs`, `tests\Linqraft.Analyzer.Tests\UnnecessaryCaptureCodeFixProviderTests.cs` |
| `LQRE001` local-variable capture | uncaptured outer values are errors, and fixes add or amend `capture:` objects | `tests\Linqraft.Analyzer.Tests\LocalVariableCaptureAnalyzerTests.cs`, `tests\Linqraft.Analyzer.Tests\LocalVariableCaptureCodeFixProviderTests.cs` |
| `LQRE002` anonymous `GroupBy` keys | anonymous grouping keys feeding `SelectExpr` are rejected and can be converted to named types | `tests\Linqraft.Analyzer.Tests\GroupByAnonymousKeyAnalyzerTests.cs`, `tests\Linqraft.Analyzer.Tests\GroupByAnonymousKeyCodeFixProviderTests.cs` |
| `LQRW001` hashed namespace usage | direct dependence on hash-based generated namespaces is warned against | `tests\Linqraft.Analyzer.Tests\GeneratedHashedNamespaceUsageAnalyzerTests.cs` |
| `LQRW002` auto-generated DTO usage | explicit dependence on auto-generated nested DTO types is warned against | `tests\Linqraft.Analyzer.Tests\AutoGeneratedDtoUsageAnalyzerTests.cs` |
