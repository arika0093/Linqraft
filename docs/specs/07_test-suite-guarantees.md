...
| --- | --- | --- |
| `LQRF001` AnonymousTypeToDto | anonymous shapes outside Linqraft contexts are suggested for DTO conversion, with context-sensitive DTO naming | `tests\Linqraft.Tests.Analyzer\AnonymousTypeToDtoAnalyzerTests.cs`, `tests\Linqraft.Tests.Analyzer\AnonymousTypeToDtoCodeFixProviderTests.cs` |
...
| `LQRF003` / `LQRF004` API response method generation | anonymous `Select`-based mock query methods can be converted into async/sync API response methods | `tests\Linqraft.Tests.Analyzer\ApiResponseMethodGeneratorAnalyzerTests.cs`, `tests\Linqraft.Tests.Analyzer\SyncApiResponseMethodGeneratorAnalyzerTests.cs`, corresponding code-fix tests |
...
| `LQRS002` anonymous `Select` to `SelectExpr` | only `IQueryable` anonymous projections are targeted, and code fixes convert them to Linqraft forms | `tests\Linqraft.Tests.Analyzer\SelectToSelectExprAnonymousAnalyzerTests.cs`, `tests\Linqraft.Tests.Analyzer\SelectToSelectExprAnonymousCodeFixProviderTests.cs` |
