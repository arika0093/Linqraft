...
| --- | --- | --- |
| `LQRF001` ApiControllerProducesResponseType | API controllers using typed `SelectExpr` can receive response-metadata suggestions and fixes | `tests\Linqraft.Tests.Analyzer\AnalyzerSmokeTests.cs`, `tests\Linqraft.Tests.Analyzer\AnalyzerCodeFixSmokeTests.cs` |
...
| `LQRS002` anonymous `Select` to `SelectExpr` | only non-null-ternary `IQueryable` anonymous projections stay hidden, and the hidden suggestion still keeps both Linqraft conversion fixes available | `tests\Linqraft.Tests.Analyzer\AnalyzerSmokeTests.cs`, `tests\Linqraft.Tests.Analyzer\AnalyzerCodeFixSmokeTests.cs` |
| `LQRS003` named `Select` to `SelectExpr` | only non-null-ternary named projections stay hidden, and the supported fix variants remain available | `tests\Linqraft.Tests.Analyzer\AnalyzerSmokeTests.cs`, `tests\Linqraft.Tests.Analyzer\AnalyzerCodeFixSmokeTests.cs` |
...
| `LQRS005` anonymous `Select` with null ternary | null-ternary anonymous projections are surfaced as informational migration candidates and reuse the same conversion fixes | `tests\Linqraft.Tests.Analyzer\AnalyzerSmokeTests.cs`, `tests\Linqraft.Tests.Analyzer\AnalyzerCodeFixSmokeTests.cs` |
| `LQRS006` named `Select` with null ternary | null-ternary named projections are surfaced as informational migration candidates and reuse the same conversion fixes | `tests\Linqraft.Tests.Analyzer\AnalyzerSmokeTests.cs`, `tests\Linqraft.Tests.Analyzer\AnalyzerCodeFixSmokeTests.cs` |
| `LQRW003` unnecessary capture | unused capture entries are diagnosed and can be removed automatically | `tests\Linqraft.Tests.Analyzer\AnalyzerSmokeTests.cs`, `tests\Linqraft.Tests.Analyzer\AnalyzerCodeFixSmokeTests.cs` |
