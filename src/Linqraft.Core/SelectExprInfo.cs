...
public abstract record SelectExprInfo
{
    ...
    {
        // Delegate to the pipeline's LINQ expression parser
        return GetPipeline().LinqExpressionParser.ExtractSelectInfo(syntax, SourceType);
    }

    private LinqExpressionInfo? ExtractSelectExprInfoFromSyntax(ExpressionSyntax syntax)
    {
        // Delegate to the pipeline's LINQ expression parser
        return GetPipeline().LinqExpressionParser.ExtractSelectExprInfo(syntax, SourceType);
    }
