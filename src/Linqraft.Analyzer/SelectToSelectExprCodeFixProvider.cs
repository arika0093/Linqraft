...
public class SelectToSelectExprCodeFixProvider : CodeFixProvider
{
        ...
        PatternInfo patternInfo)
    {
        ...
        // Pattern 2→3: Existing DTO Select → Predefined DTO SelectExpr
        if (patternInfo.IsSelect && patternInfo.UsesPredefinedDto)
        {
            context.RegisterCodeFix(
                CodeAction.Create(
                    title: "Convert to SelectExpr (keep current DTO)",
                    createChangedDocument: c => ConvertToAnonymousSelectExprAsync(context.Document, invocation, c),
                    equivalenceKey: "ConvertPredefinedToPredefinedSelectExpr"),
                diagnostic);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Get the source type from the query expression
        var expressionTypeInfo = semanticModel.GetTypeInfo(memberAccess.Expression, cancellationToken);
        if (expressionTypeInfo.Type is not INamedTypeSymbol queryableType)
            return document;

        // For IQueryable<T> or IEnumerable<T>, get T
        var sourceType = queryableType.TypeArguments.FirstOrDefault();
        if (sourceType == null)
            return document;
