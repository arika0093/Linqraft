...
public class NestedSelectExprPartialDtoAnalyzer : BaseLinqraftAnalyzer
{
    ...
    private static void AnalyzeInvocation(SyntaxNodeAnalysisContext context)
    {
        ...
        // Filter out types that are source-generated by checking their symbols
        var actuallyMissingTypes = new List<string>();
        foreach (var typeName in missingTypes)
        {
            // Try to find the type symbol to check if it's source-generated
            var typeSymbol = context.SemanticModel.Compilation.GetTypeByMetadataName(typeName);
            
            // If type symbol is null or not source-generated, it's actually missing
            if (typeSymbol == null || !DtoTypeHelper.IsSourceGeneratedDto(typeSymbol))
            {
