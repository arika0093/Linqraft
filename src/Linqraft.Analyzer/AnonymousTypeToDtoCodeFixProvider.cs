...
public class AnonymousTypeToDtoCodeFixProvider : CodeFixProvider
{
        ...
        )
        {
            ...
            )
            {
                ...
                if (lambda.Body is AnonymousObjectCreationExpressionSyntax lambdaAnonymous)
                {
                    ...
                    {
                        return expression; // Unknown lambda type
                    }

                    // Create new invocation with replaced lambda
                    var newArg = SyntaxFactory.Argument(newLambda);
                    var newArgs = invocation.ArgumentList.WithArguments(
    ...
    /// <summary>
    /// Collects all nested DTO classes from a DtoStructure
    /// </summary>
