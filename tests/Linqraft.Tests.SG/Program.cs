...
public sealed class SourceGeneratorSmokeTests
{
    ...
    public void Generator_core_allows_overriding_linqraft_specific_options()
    {
        ...
        generatedSources.Keys.Any(key => key.StartsWith("CustomMapping_", StringComparison.Ordinal))
            .ShouldBeTrue();
        generatedSources.Keys.Any(key => key.StartsWith("CustomGenerate_", StringComparison.Ordinal))
            .ShouldBeTrue();
    ...
    public void Generator_core_omits_optional_linqraft_specific_support_when_names_are_null()
    {
        ...
        generatedSources["Minimal.GlobalUsings.g.cs"]
            .ShouldContain("global using MinimalSupport;");
    }
